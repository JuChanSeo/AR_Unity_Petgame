...
public class Chapter2Script : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for(int i = 0; i< imgDisplay.Length;i++)
        {
            Mat img = new Mat(inputTexture[i].height, inputTexture[i].width, CvType.CV_8UC4);
            Utils.texture2DToMat(inputTexture[i], img);
            Texture2D outputTexture = new Texture2D(img.cols(), img.rows(), TextureFormat.RGBA32, false);
            Utils.matToTexture2D(img, outputTexture);
            imgDisplay[i].texture = outputTexture;
        }
        // Load Image 
        ...
        //Utils.matToTexture2D(img, outputTexture);
        //Utils.matToTexture2D(img2, outputTexture2);
