...
public class Petctrl : MonoBehaviour
{
	...
	public UnityEngine.UI.Slider fatigue;
	public UnityEngine.UI.Slider cleanliness;
	public UnityEngine.UI.Slider intimity;
	...
	void Update()
    {
        ...
        if (SceneManager.GetActiveScene().name == "Game3"
			&& blink_circle.activeSelf == true)
        {
			Debug.Log($"blink circle is in the scene,\t{blink_circle.transform.position}");
            ...
            {
				blink_circle.transform.position = hit_blink.point + Vector3.up * 0.1f;
			}
		...
		if (spawnedObject != null && spawnedObject.activeSelf == true
			&& SceneManager.GetActiveScene().name != "Game3" && SceneManager.GetActiveScene().name != "Tutorial_Game3")
        {

			spawnedObject.transform.eulerAngles = new Vector3(0f, spawnedObject.transform.eulerAngles.y, 0f);
			spawnedObject.transform.GetChild(0).transform.eulerAngles = new Vector3(0f, spawnedObject.transform.GetChild(0).transform.eulerAngles.y, 0f);
			if (spawnedObject.transform.GetChild(0).transform.position.y < -5f)
            {
				spawnedObject.transform.GetChild(0).transform.position =
					new Vector3(spawnedObject.transform.GetChild(0).transform.position.x,
								0, spawnedObject.transform.GetChild(0).transform.position.z);
			}

			//Debug.Log("Pome Angle: " + spawnedObject.transform.eulerAngles
			//		+ "\tPome_inva Angle: " + spawnedObject.transform.GetChild(0).transform.eulerAngles
			//		+ "\tPome_position: " + spawnedObject.transform.position
			//		+ "\tPome_inva_position: " + spawnedObject.transform.GetChild(0).transform.position);
